...
const mediaPaths = (await readFile('src/lib/media.mjs', 'utf8')).replaceAll('export ', '');
const mediaApi = (await readFile('server/media-api.mjs', 'utf8')).replace(/^import .*\n/gm, '').replaceAll('export ', '');
const worker = (await readFile('server/worker.mjs', 'utf8')).replace(/^import .*\n/gm, '').replaceAll('export ', '');
const source = `${mediaPaths}\n${api}\n${mediaApi}\n${worker}\nexport default createWorker(${JSON.stringify(assets)}, ${JSON.stringify(securityHeaders)});\n`;
...
console.log(`Worker built: ${Object.keys(assets).length} assets, /api/images and /api/media (${(Buffer.byteLength(source) / 1024 / 1024).toFixed(2)} MB).`);
